package com.myWalmart.app.ticketServiceImp;
...
import com.myWalmart.app.ticketServiceInter.TicketService;
...
public class TicketServiceImp implements TicketService {
	...
	public SeatHold findAndHoldSeats(int numSeats, Integer minLevel, Integer maxLevel, String customerEmail) {
		...
		for (int i = minLevel; i <= maxLevel; i++) {
			...
			numSeats = initialNeedSeats - sh.getSeats().size(); // update date
																// the remaining
																// needed seats
		}
		seatHolds.put(sh.getSeatHoldId(), sh);
	...
	/**
	 * Check this particular level has enough seats Zigzag search through the
	 * different rows
	 * 
	 * @param levelSeats
	 *            All seats in this level
	 * @param sh
