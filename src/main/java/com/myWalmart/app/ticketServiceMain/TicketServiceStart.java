package com.myWalmart.app.ticketServiceMain;

import java.util.Scanner;

import com.myWalmart.app.ticketServiceData.Seat;
import com.myWalmart.app.ticketServiceImp.CheckExpireTime;
import com.myWalmart.app.ticketServiceImp.TicketServiceImp;
import com.myWalmart.app.ticketServiceInter.SeatHold;
...
public class TicketServiceStart {
	...
	public static void main(String[] args) {
		while (true) {
			...
			//start a new thread for checking time out
			Thread checkTimeout = new Thread(new CheckExpireTime());
			checkTimeout.start();
	...
	private synchronized static void holdSeats() {
		...
		if (sh.getSeats().size() == seatsNum) {
			...
			for (int t = 0; t < sh.getSeats().size(); t++) {
				...
				System.out.println(" Seat Level is " + s.getLevelName() + "  row is " + s.getRow() + " seat number "
						+ s.getSeatNum());

			}

		} else if (sh.getSeats().size() < seatsNum) { // Not enough seats, set
														// all hold to false,
														// data to null
